...
class StepService : Service(), SensorEventListener {
    ...
    override fun onStartCommand(intent: Intent, flags: Int, startId: Int): Int {
        ...
        nfIntent = Intent(this, MainApp::class.java)
        setStepBuilder()
    ...
    private fun saveStepData() {
        var entity = stepData?.getCurDataByDate(currentDate!!)
        if (entity == null) {
            entity = StepEntity()
            entity.curDate = currentDate
            entity.steps = currentStep.toString()

            stepData?.addNewData(entity)
        } else {
            entity.steps = currentStep.toString()
            stepData?.updateCurData(entity)
        }
        setStepBuilder()
    }
    ...
    private fun setStepBuilder() {
        builder?.setContentIntent(
            ...
            ?.setContentTitle("Today's step " +  currentStep)
            ?.setSmallIcon(R.mipmap.ic_launcher)
            ?.setContentText("Keep Fighting! All the Best!")
